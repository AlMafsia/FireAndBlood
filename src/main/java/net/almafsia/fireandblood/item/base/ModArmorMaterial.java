...
public record ModArmorMaterial(String name, int durability, int[] protection, int enchantability, SoundEvent equipSound, float toughness, float knockbackResistance, Supplier<Ingredient> repairMaterial) implements ArmorMaterial {
    ...
    //multiplied to get the actual value
    //basically sets the analogy between the different pieces (the current is the vanilla version)
    private static final int[] DURABILITY_PER_SLOT = new int[] { 13, 15, 16, 11} ;

    @Override
    public int getDurabilityForSlot(EquipmentSlot slot) {
        return this.DURABILITY_PER_SLOT[slot.getIndex()]*this.durability;
    }
    ...
    public @NotNull String getName() {
        return FireAndBlood.MOD_ID + ":" + this.name;
    }
